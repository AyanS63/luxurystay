    ...
    <header className={`h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 fixed top-0 right-0 ${isStaff ? 'left-0' : 'left-0 md:left-64'} z-10 transition-all duration-300`}>
      {!isStaff ? (
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="relative w-full max-w-sm hidden md:block" ref={searchRef}>
            ...
            {showResults && (
              <div className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 max-h-96 overflow-auto py-2 z-20">
                {results.users.length > 0 && (
                  <div className="px-4 py-2">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Users</h4>
                    {results.users.map(user => (
                      <div key={user._id} onClick={() => handleResultClick('/dashboard/users')} className="p-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded cursor-pointer text-sm text-slate-700 dark:text-slate-200">
                ...
                {results.rooms.length > 0 && (
                  <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700">
                    ...
                    {results.rooms.map(room => (
                      <div key={room._id} onClick={() => handleResultClick('/dashboard/rooms')} className="p-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded cursor-pointer text-sm text-slate-700 dark:text-slate-200 flex justify-between">
                ...
                {results.bookings.length > 0 && (
                  <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700">
                    ...
                    {results.bookings.map(booking => (
                      <div key={booking._id} onClick={() => handleResultClick('/dashboard/bookings')} className="p-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded cursor-pointer text-sm text-slate-700 dark:text-slate-200">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 py-1 z-20 overflow-hidden animate-fade-in-up">
