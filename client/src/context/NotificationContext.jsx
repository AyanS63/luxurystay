...
export const NotificationProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (['admin', 'manager', 'receptionist'].includes(user.role)) {
      ...
      socket.on('new_booking', (data) => handleEvent('booking', data));
      socket.on('new_event', (data) => handleEvent('event', data));
      socket.on('new_inquiry', (data) => handleEvent('inquiry', data));
      socket.on('check_in', (data) => handleEvent('check_in', data));
      socket.on('check_out', (data) => handleEvent('check_out', data));
      socket.on('payment_received', (data) => handleEvent('payment_received', data));
      socket.on('payment_reversed', (data) => handleEvent('payment_reversed', data));
      ...
      return () => {
        ...
        socket.off('check_in');
        socket.off('check_out');
        socket.off('payment_received');
        socket.off('payment_reversed');
      };
    }
  }, [socket, user?.role]);
