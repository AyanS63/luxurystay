...
export const createPaymentIntent = async (req, res) => {
  try {
    ...
    if (existingUserBooking) {
      return res.status(400).json({ message: 'You already have an active booking for this room' });
    }
...
export const createBooking = async (req, res) => {
  try {
    const { room, checkInDate, checkOutDate, guests, specialRequests, extras = [], paymentIntentId } = req.body;
    ...
    if (overlappingBooking) {
      ...
      return res.status(400).json({ message: 'Room was booked by someone else while you were paying. Please contact support for refund.' });
    }
    ...
    if (Array.isArray(extras)) {
        ...
        extrasTotal = sanitizedExtras.reduce((sum, item) => sum + (item.price || 0), 0);
    }
    ...
    const newBilling = new Billing({
        ...
        items: [
            ...
            ...sanitizedExtras.map(e => ({ description: `Extra: ${e.name}`, amount: e.price }))
        ]
    });
...
export const updateBookingStatus = async (req, res) => {
  try {
    ...
    if (['Confirmed', 'CheckedIn'].includes(status)) {
        ...
        // Emit Check-in Event
        if (status === 'CheckedIn' && req.io) {
             req.io.emit('check_in', {
    ...
    // If booking is finished or invalid, free up the room
        ...
        if (status === 'CheckedOut' && req.io) {
             req.io.emit('check_out', {
        ...
        // REFUND LOGIC
        if (['Cancelled', 'Rejected'].includes(status) && booking.paymentIntentId) {
            try {
